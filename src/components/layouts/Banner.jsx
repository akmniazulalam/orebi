              ...
              <div
                ...
                <div className="relative w-full 2xl:max-w-315 xl:max-w-5xl lg:max-w-3xl md:max-w-xl sm:max-w-xl xs:max-w-xs 2xs:max-w-sm mx-auto">
                  <div className="max-w-[85%] sm:max-w-md md:max-w-lg lg:max-w-138 py-12 sm:py-16 md:py-20 xs:space-y-2.5 md:space-y-2.5 lg:space-y-5 relative z-10">
                    ...
                    <h2 className="xs:text-base sm:text-2xl md:text-3xl lg:text-[40px] xl:text-6xl font-bold tracking-tight text-menuHeading dark:text-white leading-none md:leading-[1.14] font-dmSans">
                      {slide.title}
                    </h2>

                    <p className="xs:text-[10px] sm:text-sm md:text-base text-menuHeading dark:text-slate-300 sm:leading-[1.4] font-medium font-dmSans max-w-xs sm:max-w-sm md:max-w-111.5">
                      {slide.description}
                    </p>

                    <div className="sm:pt-1 md:pt-3">
                      <Link
                        to={slide.link}
                        className="inline-flex items-center gap-1 md:gap-2 rounded-full bg-menuHeading text-white dark:bg-white dark:text-[#262626] px-2 xs:px-3 md:px-5 py-1 sm:py-1 md:py-2.5 lg:py-3 xs:text-[8px] sm:text-[12px] font-bold font-dmSans shadow-md hover:bg-black/85 dark:hover:bg-white/90 hover:shadow-lg transition-all duration-300 group focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2">
                        <span>{slide.buttonText}</span>
                        <ArrowRight className="h-2 w-2 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4 transition-transform duration-300 group-hover:translate-x-1" />
                      </Link>
                    </div>
                  </div>

                  {/* Custom Progress Indicator (Vertical on Desktop, Horizontal on Mobile) */}
                  <div className="absolute md:-left-14 lg:-left-12 xl:-left-15 bottom-5 sm:bottom-8 md:bottom-auto md:top-1/2 md:-translate-y-1/2 flex flex-row md:flex-col items-center md:items-start gap-2.5 md:gap-y-3 z-20 pointer-events-auto">
